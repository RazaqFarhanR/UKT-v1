...
const pengurus_ranting = () => {
    ...
    // state
    const [dataPengurusRanting, setDataPengurusRanting] = useState ([])
    const [action, setAction] = useState ('')
    const [idPengurusRanting, setIdPengurusRanting] = useState ('')
    ...
    // function modal add
    const addModal = () => {
        setShowModalPengurusRanting (true)
        setAction ('insert')
        setNiw ('')
        setName ('')
        setJabatan ('')
    ...
    const editModal = (selectedItem) => {
        setShowModalPengurusRanting (true)
        setAction ('update')
        setIdPengurusRanting (selectedItem.id_pengurus)
    ...
    const deleteModal = (selectedId) => {
        ...
        setAction ('deletePengurusRanting')
    ...
    return (
        <>
                    ...
                    <div className="min-h-full bg-darkBlue p-6">
                        ...
                        <div className="flex justify-between items-center text-white mb-7">
                            ...
                            <div className="flex justify-center items-center gap-x-3">
                                <Link href={'./pengurus'} className="bg-purple hover:bg-white rounded-md w-9 h-9 flex justify-center items-center group duration-300">
                                ...
                                <h1 className='text-2xl tracking-wider'>Pengurus Ranting</h1>
                            </div>
                        ...
                        <div className="bg-navy rounded-md py-2 px-3">
                            ...
                            <table className='w-full table-fixed'>
                                ...
                                <tbody>
                                    {dataPengurusRanting.filter (a => a.id_role === 'pengurus ranting').map((item, index) => (
                                        <tr key={index + 1} className='text-white text-center'>
                                            ...
                                            <td className='border-b-2 border-gray'>{item.jabatan}</td>
                                            ...
                                            <td className='border-b-2 border-gray'>
                                                <div className="flex gap-x-2">
                                                    ...
                                                    <button onClick={() => deleteModal(item.id_pengurus)} className="bg-red hover:bg-white text-white hover:text-red py-2 rounded-md w-28 flex justify-center items-center space-x-[1px] mx-auto group duration-300">
            ...
            {/* memanggil modal */}
            <globalState.Provider value={{ showModalPengurusRanting, setShowModalPengurusRanting, dataPengurusRanting, setDataPengurusRanting, action, setAction, idPengurusRanting, niw, setNiw, name, setName, jabatan, setJabatan, ranting, setRanting, username, setUsername, password, setPassword, role, foto, setFoto }}>
                <Modal_pengurus_ranting />
            </globalState.Provider>

            <globalState.Provider value={{ showModalDelete, setShowModalDelete, setDataPengurusRanting, action, idPengurusRanting }}>
                <Modal_delete />
            </globalState.Provider>
        </>
    )
...
export default pengurus_ranting