...
const modal_pengurus_ranting = () => {
    ...
    //state modal
    const {showModalPengurusRanting, setShowModalPengurusRanting} = useContext (globalState)

    // state
    const {setDataPengurusRanting, action, idPengurusRanting} = useContext (globalState)
    const {niw, setNiw} = useContext (globalState)
    const {name, setName} = useContext (globalState)
    const {jabatan, setJabatan} = useContext (globalState)
                        ...
                        <div className="relative bg-navy text-white rounded-lg shadow">
                            <form action="POST" onSubmit={handleSave}>
                                ...
                                <div className="flex justify-center p-4">
                                    ...
                                    <button onClick={() => setShowModalPengurusRanting(false)} type="button" className="p-1.5 inline-flex items-center absolute right-5">
                                ...
                                <div className="flex items-center justify-end p-6 space-x-2">
                                    <button onClick={() => setShowModalPengurusRanting(false)} className="text-red hover:text-white bg-white hover:bg-red duration-300 font-medium rounded-lg px-5 py-2.5 text-center">Cancel</button>
                                    {(() => {
                                        if (action === 'insert') {
                                            return (
...
export default modal_pengurus_ranting