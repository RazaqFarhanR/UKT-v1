...
import Modal_ukt_teknik from '../components/modal_ukt_teknik'
import Modal_delete from '../components/modal_delete'
const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL;

const detail_teknik_hijau = () => {

    // state modal
    const [showModalUktTeknik, setShowModalUktTeknik] = useState (false)
    ...
    const [teknikName, setTeknikName] = useState ('')

    // function get data fisik
    const getDataTeknik = () => {
        const token = localStorage.getItem ('token')

        axios.get (BASE_URL + `teknik/ukt/UKT Hijau`, { headers: { Authorization: `Bearer ${token}`}})
        .then (res => {
    ...
    const editModal = (selectedItem) => {
        ...
        setAction ('update')
        setUkt ('UKT Hijau')
        setIdTeknik (selectedItem.id_teknik)
        setTeknikName (selectedItem.name)
    }
    ...
    return (
        <>
                    ...
                    <div className="min-h-full bg-darkBlue p-6">
                        ...
                        <div className="flex justify-between items-center text-white mb-7">
                            ...
                            <div className="flex justify-center items-center gap-x-3">
                                ...
                                <h1 className='text-2xl tracking-wider'>Teknik - UKT Jambon</h1>
                            </div>
                        ...
                        <div className="space-y-3">
                            ...
                            {dataTeknik.map ((item, index) => (
                                <div key={index + 1} className="grid grid-cols-12 gap-x-3 text-white font-semibold text-lg">
                                    ...
                                    <div className="col-span-2 bg-navy rounded-md flex items-center justify-center gap-x-2">
                                        <button onClick={() => editModal(item)} className="bg-green hover:bg-white duration-300 p-1.5 rounded-md flex items-center group">
            ...
            {/* memanggil modal */}
            <globalState.Provider value={{ showModalUktTeknik, setShowModalUktTeknik, action, dataTeknik, setDataTeknik, idTeknik, teknikName, setTeknikName, ukt, setUkt }}>
                <Modal_ukt_teknik />
            </globalState.Provider>

            <globalState.Provider value={{ showModalDelete, setShowModalDelete, action, dataTeknik, setDataTeknik, ukt, idTeknik }}>
                <Modal_delete />
            </globalState.Provider>
        </>
    )
...
export default detail_teknik_hijau